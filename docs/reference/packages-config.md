...
使用する場合`packages.config` 、は通常、プロジェクトのルートにあります。 これは、最初の NuGet 操作の実行時に自動的に作成されますが、など`nuget restore`のコマンドを実行する前に手動で作成することもできます。
