...
使用する場合`packages.config`通常はプロジェクトのルートに配置します。 最初の NuGet 操作の実行がなど任意のコマンドを実行する前に手動作成することも自動的に作成された`nuget restore`します。

使用するプロジェクト[PackageReference](../consume-packages/Package-References-in-Project-Files.md)使用しないでください`packages.config`します。
