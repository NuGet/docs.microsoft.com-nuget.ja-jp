...
2. [Andres Aragoneses](https://www.codeplex.com/site/users/view/knocte) ( [@knocte](https://twitter.com/knocte) )
    - [#2865](https://nuget.codeplex.com/workitem/2865) - `NuGet.targets` 大文字と小文字を区別する OS のスペル修正
...
5. [Olivier Dagenais](https://www.codeplex.com/site/users/view/OliIsCool) ( [@OliIsCool](https://twitter.com/oliiscool) )
    - [#2920](https://nuget.codeplex.com/workitem/2920) nuget.exe パックのコマンドでは、プロパティが MSBuild に反映されません
...
' nuget.exe update ' と ' FileConflictAction ' の両方に、コマンドラインシナリオに既定値を設定する新しいオプション '-' が追加されました。
...
NuGet パッケージの最上位レベルで、新しい従来のフォルダーが作成されています。  、、およびのピアとして、 `\lib` `\content` `\tools` パッケージにフォルダーを含めることができるようになりました `\build` 。  このフォルダーには、固定名またはの2つのファイルを配置でき `{packageid}.targets` `{packageid}.props` ます。 これらの2つのファイルは、 `build` 他のフォルダーと同じように、フレームワーク固有のフォルダーの下に配置できます。 最も一致するフレームワークフォルダーを選択するルールは、これらのフォルダーとまったく同じです。

NuGet が \ ビルドファイルを含むパッケージをインストールすると、ファイル `<Import>` とファイルを指す MSBuild 要素がプロジェクトファイルに追加され `.targets` `.props` ます。 ファイルが一番上に追加され、 `.props` `.targets` ファイルが一番下に追加されます。

### <a name="specify-different-references-per-platform-using-references-element"></a>要素を使用してプラットフォームごとに異なる参照を指定する `<References/>`

2.5 より前のファイルでは、 `.nuspec` ユーザーは参照ファイルを指定するだけで、すべてのフレームワークに追加することができます。 2.5 のこの新機能により、ユーザーは `<reference/>` サポートされているプラットフォームごとに要素を作成できます。たとえば、次のようになります。
...
ファイルに基づいて NuGet がプロジェクトに参照を追加する方法のフローを次に示し `.nuspec` ます。

1. `lib`ターゲットフレームワークに適したフォルダーを探し、そのフォルダーからアセンブリの一覧を取得します。
...
この新機能により、パッケージの作成者は参照機能を使用して、複数のフォルダーで重複するアセンブリを実行する必要がある場合に、アセンブリのサブセットを異なるフレームワークに適用でき `lib` ます。
...
### <a name="improved-project-reference-support-for-nugetexe-pack"></a>nuget.exe パックのプロジェクト参照サポートの向上

ここで nuget.exe pack コマンドは、次の規則で参照されるプロジェクトを処理します。

1. 参照先のプロジェクトに対応するファイルがある場合 `.nuspec` (たとえば、と同じフォルダーにという名前のファイルが存在する場合)、 `proj1.nuspec` `proj1.csproj` ファイルから読み取られた id とバージョンを使用して、このプロジェクトがパッケージへの依存関係として追加されます。 `.nuspec`
...
これにより、ファイルが存在する場合、参照先のプロジェクトを依存関係として扱うことができ `.nuspec` ます。それ以外の場合は、パッケージの一部になります。

詳細については、こちらをご覧ください。 [http://nuget.codeplex.com/workitem/936](http://nuget.codeplex.com/workitem/936)
...
この機能を使用すると、パッケージの作成者は、特定のバージョンの NuGet の後でのみパッケージを使用するように指定できます。 `.nuspec`Nuget 2.5 の後に新しい機能が追加されると、パッケージは最小限の nuget バージョンを要求できるようになります。
...
### <a name="nugetexe-outputs-http-requests-with-detailed-verbosity"></a>nuget.exe は、詳細な詳細度で http 要求を出力します
...
### <a name="nugetexe-push-now-supports-unc-and-folder-sources"></a>nuget.exe プッシュで UNC およびフォルダーソースがサポートされるようになりました

NuGet 2.5 より前では、UNC パスまたはローカルフォルダーに基づいて "nuget.exe push" をパッケージソースに対して実行しようとすると、プッシュは失敗します。 最近追加された階層構成機能を使用すると、nuget.exe が UNC/フォルダーソースまたは HTTP ベースの NuGet ギャラリーをターゲットにする必要があることがよくありました。

NuGet 2.5 以降、nuget.exe が UNC/フォルダーソースを識別する場合、ソースへのファイルコピーを実行します。
...
### <a name="nugetexe-supports-explicitly-specified-config-files"></a>明示的に指定された構成ファイルをサポート nuget.exe

構成にアクセスする nuget.exe コマンド (' spec ' と ' pack ' を除く) は、新しい '-ConfigFile ' オプションをサポートするようになりました。これにより、% AppData% \nuget\Nuget.Config の既定の構成ファイルの代わりに特定の構成ファイルが強制的に使用されます。
