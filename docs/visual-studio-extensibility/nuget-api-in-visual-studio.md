...
長年にわたって、NuGet には多くのサービスが追加されてきましたが、それらはすべて `NuGet.VisualStudio.dll` アセンブリの `NuGet.VisualStudio` 名前空間内にあります。

NuGet 3.3 以降、NuGet によって以下がエクスポートされます

- [`IRegistryKey`](#iregistrykey-interface):レジストリ サブキーから値を取得するメソッド。 (3.3 以降)
- [`IVsCredentialProvider`](#ivscredentialprovider-interface) NuGet 操作の資格情報を取得するメソッドが含まれています。 (4.0 以降)
- [`IVsFrameworkCompatibility`](#ivsframeworkcompatibility-interface) フレームワークとフレームワーク間の互換性を検出するメソッドが含まれています。 (4.0 以降)
- [`IVsFrameworkCompatibility2`](#ivsframeworkcompatibility2-interface) フレームワークとフレームワーク間の互換性を検出するメソッドが含まれています。 (4.0 以降)
- [`IVsFrameworkCompatibility3`](#ivsframeworkcompatibility3-interface) フレームワークとフレームワーク間の互換性を検出するメソッドが含まれています。 (5.8 以降)
- [`IVsFrameworkParser`](#ivsframeworkparser-interface) 文字列と [FrameworkName](/dotnet/api/system.runtime.versioning.frameworkname) 間の変換を処理するためのインターフェイス (4.0 以降)
- [`IVsFrameworkParser2`](#ivsframeworkparser2-interface) .NET Framework 文字列を解析するインターフェイス。 [NuGet-IVsFrameworkParser](https://aka.ms/NuGet-IVsFrameworkParser) のページを参照してください。 (5.8 以降)
- [`IVsGlobalPackagesInitScriptExecutor`](#ivsglobalpackagesinitscriptexecutor-interface) ソリューション内のパッケージから PowerShell スクリプトを実行します (4.0 以降)
- [`IVsNuGetFramework`](#ivsnugetframework-interface) .NET ターゲット フレームワーク モニカーのコンポーネントを表す型。 (5.8 以降)
- [`IVsPackageInstaller`](#ivspackageinstaller-interface):プロジェクトに NuGet パッケージをインストールするメソッド。 (3.3 以降)
- [`IVsPackageInstaller2](#ivspackageinstaller2-interface) 現在のソリューション内のプロジェクトに 1 つのパッケージの最新バージョンをインストールするメソッドが含まれています。
- [`IVsPackageInstallerEvents`](#ivspackageinstallerevents-interface):パッケージのインストールまたはアンインストールのイベント。 (3.3 以降)
- [`IVsPackageInstallerProjectEvents`](#ivspackageinstallerprojectevents-interface):パッケージのインストールまたはアンインストールのバッチ イベント。 (3.3 以降)
- [`IVsPackageInstallerServices`](#ivspackageinstallerservices-interface):現在のソリューションにインストールされているパッケージを取得し、プロジェクトで特定のパッケージがインストールされているかどうかを確認するメソッド。 (3.3 以降)
- [`IVsPackageManagerProvider`](#ivspackagemanagerprovider-interface):NuGet パッケージの代替パッケージ マネージャー候補を提供するメソッド。 (3.3 以降)
- [`IVsPackageMetadata`](#ivspackagemetadata-interface):インストールされているパッケージに関する情報を取得するメソッド。 (3.3 以降)
- [`IVsPackageProjectMetadata`](#ivspackageprojectmetadata-interface):NuGet のアクションが実行されているプロジェクトに関する情報を取得するメソッド。 (3.3 以降)
- [`IVsPackageRestorer`](#ivspackagerestorer-interface):プロジェクトにインストールされているパッケージを復元するメソッド。 (3.3 以降)
- [`IVsPackageSourceProvider`](#ivspackagesourceprovider-interface):NuGet パッケージ ソースのリストを取得するメソッド。 (3.3 以降)
- [`IVsPackageUninstaller`](#ivspackageuninstaller-interface):プロジェクトから NuGet パッケージをアンインストールするメソッド。 (3.3 以降)
- [`IVsPathContext`](#ivspathcontext-interface) 現在のコンテキスト (プロジェクト コンテキストなど) に固有の NuGet パス情報。 (4.0 以降)
- [`IVsPathContext2`](#ivspathcontext2-interface) 現在のコンテキスト (プロジェクト コンテキストなど) に固有の NuGet パス情報。 (5.0 以降)
- [`IVsPathContextProvider`](#ivspathcontextprovider-interface) [IVsPathContext](#ivspathcontext-interface) インスタンスを初期化するファクトリ。 (4.0 以降)
- [`IVsPathContextProvider2`](#ivspathcontextprovider2-interface) [IVsPathContext2](#ivspathcontext2-interface) インスタンスを初期化するファクトリ。 (5.0 以降)
- [`IVsProjectJsonToPackageReferenceMigrateResult`](#ivsprojectjsontopackagereferencemigrateresult-interface) レガシ project.json プロジェクトに対する移行操作の結果が含まれています (4.3 以降)
- [`IVsProjectJsonToPackageReferenceMigrator`](#ivsprojectjsontopackagereferencemigrator-interface) project.json ベースのレガシ プロジェクトを PackageReference ベースのプロジェクトに移行するメソッドが含まれています。 (4.3 以降)
- [`IVsSemanticVersionComparer`](#ivssemanticversioncomparer-interface) 2 つの不透明なバージョン文字列を NuGet セマンティックとして扱うことで比較するインターフェイス (4.0 以降)
- [`IVsTemplateWizard`](#ivstemplatewizard-interface):事前インストールされているパッケージを含めるプロジェクトまたは項目テンプレート用に設計。このインターフェイスはコードから呼び出されることを意図したもの "*ではない*" ため、パブリック メソッドはありません。 (3.3 以降)
...
1. サービスを使用するには、[MEF インポート属性](/dotnet/framework/mef/index#imports-and-exports-with-attributes)または [IComponentModel サービス](/dotnet/api/microsoft.visualstudio.componentmodelhost.icomponentmodel)を通じてそのサービスをインポートします。
...
## <a name="ivscredentialprovider-interface"></a>IVsCredentialProvider インターフェイス
...
## <a name="ivsframeworkcompatibility-interface"></a>IVsFrameworkCompatibility インターフェイス
...
## <a name="ivsframeworkcompatibility2-interface"></a>IVsFrameworkCompatibility2 インターフェイス
...
## <a name="ivsframeworkcompatibility3-interface"></a>IVsFrameworkCompatibility3 インターフェイス
...
## <a name="ivsframeworkparser-interface"></a>IVsFrameworkParser インターフェイス
...
## <a name="ivsframeworkparser2-interface"></a>IVsFrameworkParser2 インターフェイス
...
## <a name="ivsglobalpackagesinitscriptexecutor-interface"></a>IVsGlobalPackagesInitScriptExecutor インターフェイス
...
## <a name="ivsnugetframework-interface"></a>IVsNuGetFramework インターフェイス
...
## <a name="ivspackageinstaller2-interface"></a>IVsPackageinstaller2 インターフェイス
...
## <a name="ivspathcontext-interface"></a>IVsPathContext インターフェイス
...
## <a name="ivspathcontext2-interface"></a>IVsPathContext2 インターフェイス
...
## <a name="ivspathcontextprovider-interface"></a>IVsPathContextProvider インターフェイス
...
## <a name="ivspathcontextprovider2-interface"></a>IVsPathContextProvider2 インターフェイス
...
## <a name="ivsprojectjsontopackagereferencemigrateresult-interface"></a>IVsProjectJsonToPackageReferenceMigrateResult インターフェイス
...
## <a name="ivsprojectjsontopackagereferencemigrator-interface"></a>IVsProjectJsonToPackageReferenceMigrator インターフェイス
...
## <a name="ivssemanticversioncomparer-interface"></a>IVsSemanticVersionComparer インターフェイス
