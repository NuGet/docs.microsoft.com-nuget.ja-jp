...
RegistrationsBaseUrl/3.0.0-ベータ | `RegistrationsBaseUrl` のエイリアス
RegistrationsBaseUrl/3.0.0   | `RegistrationsBaseUrl` のエイリアス
...
名  | [種類]             | 必要 | ノート
----- | ---------------- | -------- | -----
count | 整数          | 可      | インデックス内の登録ページ数
品目 | オブジェクトの配列 | 可      | 登録ページの配列
...
count  | 整数          | 可      | ページ内の登録リーフの数
品目  | オブジェクトの配列 | Ｘ       | 登録リーフの配列とそれらのメタデータの関連付け
方  | string           | 可      | ページ内の最小の SemVer 2.0.0 バージョン (包括的)
所属 | string           | Ｘ       | 登録インデックスの URL
上限  | string           | 可      | ページ内の最大 SemVer 2.0.0 バージョン (包括)
...
catalogEntry   | オブジェクト | 可      | パッケージメタデータを含むカタログエントリ
パッケージのパッケージ | string | 可      | パッケージコンテンツへの URL (. nupkg)
...
今後              | オブジェクト                     | Ｘ       | パッケージに関連付けられている非推奨
説明              | string                     | Ｘ       | 
iconUrl                  | string                     | Ｘ       | 
ID                       | string                     | 可      | パッケージの ID
licenseUrl               | string                     | Ｘ       |
licenseExpression        | string                     | Ｘ       | 
一覧                   | boolean                    | Ｘ       | 存在しない場合は、一覧として考慮する必要があります
minClientVersion         | string                     | Ｘ       | 
projectUrl               | string                     | Ｘ       | 
投稿                | string                     | Ｘ       | パッケージが発行されたときの ISO 8601 タイムスタンプを含む文字列
...
`licenseExpression` プロパティの値は、 [NuGet ライセンス式の構文](https://docs.microsoft.com/nuget/reference/nuspec#license)に準拠しています。
...
range        | オブジェクト | Ｘ       | 依存関係の許可されている[バージョン範囲](../concepts/package-versioning.md#version-ranges-and-wildcards)
登録 | string | Ｘ       | この依存関係の登録インデックスの URL

`range` プロパティが除外されている場合、または空の文字列の場合、クライアントは既定で `(, )`バージョン範囲を指定する必要があります。 つまり、依存関係のすべてのバージョンが許可されます。
...
登録ページに登録リーフが含まれています。 登録ページを取得する URL は、前述の[登録ページオブジェクト](#registration-page-object)の `@id` プロパティによって決定されます。

`items` 配列が登録インデックスに指定されていない場合、`@id` 値の HTTP GET 要求は、そのルートとしてオブジェクトを含む JSON ドキュメントを返します。 オブジェクトには、次のプロパティがあります。

名   | [種類]             | 必要 | ノート
------ | ---------------- | -------- | -----
@id    | string           | 可      | 登録ページの URL
count  | 整数          | 可      | ページ内の登録リーフの数
品目  | オブジェクトの配列 | 可      | 登録リーフの配列とそれらのメタデータの関連付け
方  | string           | 可      | ページ内の最小の SemVer 2.0.0 バージョン (包括的)
所属 | string           | 可      | 登録インデックスの URL
上限  | string           | 可      | ページ内の最大 SemVer 2.0.0 バージョン (包括)
...
登録リーフをフェッチする URL は、登録インデックスまたは登録ページの登録リーフオブジェクトの `@id` プロパティから取得されます。
...
一覧         | boolean | Ｘ       | 存在しない場合は、一覧として考慮する必要があります
パッケージのパッケージ | string  | Ｘ       | パッケージコンテンツへの URL (. nupkg)
投稿      | string  | Ｘ       | パッケージが発行されたときの ISO 8601 タイムスタンプを含む文字列
登録   | string  | Ｘ       | 登録インデックスの URL
